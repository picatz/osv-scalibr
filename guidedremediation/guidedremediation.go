...
func computeVulnsResultsLockfile(resolved remediation.ResolvedGraph, allPatches []result.Patch) []result.Vuln {
	...
	for _, p := range allPatches {
		for _, v := range p.Fixed {
			for _, pkg := range v.Packages {
				fixableVulns[vuln{id: v.ID, pkgName: pkg.Name, pkgVersion: pkg.Version}] = struct{}{}
			}
		}
	}
	...
	for _, v := range resolved.Vulns {
		...
		for vk := range vks {
			_, fixable := fixableVulns[vuln{id: v.OSV.ID, pkgName: vk.Name, pkgVersion: vk.Version}]
	...
	slices.SortFunc(vulns, func(a, b result.Vuln) int {
		...
		if c := strings.Compare(a.Packages[0].Name, b.Packages[0].Name); c != 0 {
			return c
		}
		return strings.Compare(a.Packages[0].Version, b.Packages[0].Version)
	})
	return vulns
}
